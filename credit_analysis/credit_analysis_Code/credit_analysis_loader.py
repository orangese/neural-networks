...
def load_file(file, file_length, num_independent, test_to_train):
  ...
  data = {"train": big_data[0:num_train], "validation": big_data[num_train:num_validation + num_train], "test": big_data[num_validation:]}
...
if __name__ == "__main__":
  data = load_data()
  with open("creditapproval_p.txt", "w") as filestream:
    for (independents, label) in data["train"]:
      for var in independents:
