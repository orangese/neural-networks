...
class Layer(object):
  ...
  def __init__(self, dim):
    self.dim = dim
    self.output = np.random.normal(loc = 0, size = self.dim)
...
class Conv(Layer):
  ...
  def backprop(self):
    ...
    if isinstance(self.next_layer, Pooling):
      ...
      next_error = self.next_layer.error.reshape(*self.next_layer.dim, 1)
...
##      for fmap_num in range(self.num_fmaps):
...
class Pooling(Layer):
  ...
  def get_loc_fields(self, a):
    """divides the convolutional output into local fields to prepare for \
    pooling"""
...
##    for fmap_num in range(self.previous_layer.num_fmaps):
...
def generate_zero_data():
  ...
  target[0] = 1.0
  data["train"] = [(np.ones((28, 28)), target) for i in range(10000)]
...
def test(net_type = "conv", data = None, test_acc = False, test_cost = False):
  ...
  if net_type == "conv":
    net = Network([Layer((28, 28)), Conv((5, 5), 20), Pooling((2, 2)),
                   Dense(10, actv = Activation("sigmoid"))],
                  cost = Cost("cross-entropy", reg_parameter = 0.0))
  elif net_type == "mlp":
